<template>
  <div class="flex items-center gap-4 p-4 rounded-lg bg-zinc-800/50 hover:bg-zinc-800/70 transition-colors">
    <span class="text-2xl font-bold text-zinc-500 w-12 text-center">{{ rank }}</span>
    <img v-if="user?.profilePictureObjectId" :src="useObject(user.profilePictureObjectId)" class="size-12 rounded-full" />
...
defineProps<{ rank: number; user?: { id?: string; username?: string; displayName?: string; profilePictureObjectId?: string }; playtimeHours: number; achievements: number; gamesOwned: number }>();
</script>