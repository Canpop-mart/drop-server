<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold font-display text-zinc-100 mb-8">{{ $t('community.title') }}</h1>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      ...
      <div class="p-4 bg-zinc-800/50 rounded text-center">
        <p class="text-2xl font-bold text-blue-400">{{ stats?.totalPlaytimeHours||0 }}{{ $t('community.stats.hoursSuffix') }}</p>
        <p class="text-sm text-zinc-400">{{ $t('community.stats.totalPlaytime') }}</p>
      </div>
      <div class="p-4 bg-zinc-800/50 rounded text-center">
        <p class="text-2xl font-bold text-blue-400">{{ stats?.pendingRequests||0 }}</p>
        <p class="text-sm text-zinc-400">{{ $t('community.stats.pendingRequests') }}</p>
      </div>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2">
        <h2 class="text-xl font-bold font-display text-zinc-100 mb-4">{{ $t('community.recentActivity') }}</h2>
        <ActivityFeed :items="activity" />
      </div>
      <div class="lg:col-span-1">
        <h2 class="text-xl font-bold font-display text-zinc-100 mb-4">{{ $t('community.topPlayers') }}</h2>
        <div class="space-y-2">
