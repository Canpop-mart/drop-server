...
export default defineEventHandler(async (h3) => {
  ...
  const achievementMap = Object.fromEntries(
    achievementRecords.map((a) => [a.id, a]),
  );

  // Fetch game info for achievements too
  const achievementGameIds = [
    ...new Set(achievementRecords.map((a) => a.gameId)),
  ];
  ...
  const achievementGameMap = Object.fromEntries(
    achievementGames.map((g) => [g.id, g]),
  );
  ...
  return {
    ...
    achievements: achievements.map((a) => {
      ...
      return {
        ...
        game: achievement
          ? (achievementGameMap[achievement.gameId] ?? null)
          : null,
      };
    }),
  };
